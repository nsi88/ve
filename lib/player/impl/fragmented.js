...
Fragmented.prototype.onSourceOpen = function(e) {
  ...
  ['video', 'audio'].forEach(function(track) {
    if (_this.src[track + 'InitFragment']) {
      _this.readFileByPath(_this.src[track + 'InitFragment'], function(err, res) {
        if (err) {
          throw err;
        } else {
          var sourceBuffer = _this.mediaSource.addSourceBuffer('video/mp4; codecs="' + _this.src[track + 'Codec'] + '"');
          sourceBuffer.track = track;
